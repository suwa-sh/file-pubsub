...
| サービス | 役割 |
|---|---|
| `file-pubsub` | 本体。`config.yaml` の 3 topic (orders=SFTP / customers=FTP / invoices=local) を 5 秒間隔でポーリング |
...
producer 役はホストの `sources/` 配下にファイルを置くだけです。配信結果 (Archive / Manifest / Subscription 配下の複製) はホストの `data/` 配下で確認できます。
...
- 収集元 (`sources\sftp` 等) のファイルは GET 後 DELETE (既定) で回収され、消えています。
