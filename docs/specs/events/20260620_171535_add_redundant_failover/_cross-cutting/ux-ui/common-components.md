...
| [C-08](#c-08-lockmanager) | LockManager | L-daemon-gateway | Lock(lease)の取得 / 奪取 / heartbeat / 解放 / stale 判定・回復 | 3 |
...
- **責務**: `manifest/{message_id}.json` の読み書き(AtomicWriter 利用)、Subscription 別配送状態(delivered / failed / dlq)・リトライ回数・配送日時の更新、配送イベント追記、Replay 記録の追記、状態・topic・subscription による検索を提供する。配送状態の正は常に Manifest(CTR-003)。状態遷移・冪等判定のロジック自体は DeliveryStateMachine(C-05)に委譲し、本コンポーネントは永続化と照会に徹する。**冗長構成(2 active が同時に同一 message_id を更新しうる)向けに、単純な全体上書き(`Put` の単純 AtomicWrite)とは役割を分け、`PutMerged`(message_id 更新ロック + read-merge-write + 世代 CAS + 競合リトライ)を提供する**: まず同一 message_id の更新ロック(`manifest/{message_id}.json.lock` を O_CREATE\|O_EXCL で取得)を取り、同一 message_id への 2 active 同時更新を直列化する(主機構。取得失敗はリトライ/バックオフ、上限超過は fail-closed)。ロック保持下で対象 Manifest を再読込して世代(mtime または revision)を観測し、Subscription 別配送状態を **merge precedence(決着状態 delivered/dlq は保持・上書き不可、中間状態 failed は再配信成功時に delivered へ昇格可)** でマージ、一時名で書いて rename 直前に世代を再確認(ロック下の二重チェック)し、一致時のみ rename を確定してロックを解放する。不一致(NFS でロックの原子性が破れ他 active が更新)なら再読込して有限回リトライし、決着状態(delivered/dlq)の lost update を防ぐ(spec-decision-011)。**既知の制約**: NFS では O_CREATE\|O_EXCL・read/write の原子性が実装依存で完全な分散排他は保証できず、本機構は『実務上の原子性 + 被害限定』で exactly-once は保証しない(破損なし・被害は重複配信に限定)。`Put` は新規作成・単一 writer 文脈の上書き用、`PutMerged`/`RecordDelivery` は配送状態更新用と使い分ける
- **配置レイヤー**: `L-daemon-gateway`
- **利用する UC**:
...
func (s *ManifestStore) Put(m *Manifest) error // 新規作成・単一 writer 文脈の AtomicWriter 上書き
// PutMerged は message_id 更新ロック + read-merge-write + 世代 CAS + 競合リトライ で配送状態を更新する。
// まず manifest/{message_id}.json.lock を O_CREATE|O_EXCL で取得して同一 message_id 更新を直列化し (主機構)、
// ロック保持下で read→mutate(merge precedence)→ 一時名書込 → rename 直前に世代 (mtime/revision) 再確認 → rename 確定 → ロック解放、を行う。
// 世代不一致 (NFS でロックが破れ他 active が更新) なら再読込して mutate を再適用し有限回リトライする (spec-decision-011)。
// merge precedence: 決着状態 delivered/dlq は上書き不可、中間状態 failed は delivered へ昇格可。
// 2 active が同一 message_id を同時更新しても delivered/dlq の決着状態を取りこぼさない。
// 既知の制約: NFS の原子性は実装依存で完全な分散排他は保証せず被害限定に留める (exactly-once は保証しない)
func (s *ManifestStore) PutMerged(messageID string, mutate func(*Manifest) error) error
func (s *ManifestStore) Find(f Filter) ([]*Manifest, error) // topic / subscription / status 絞り込み
// RecordDelivery は PutMerged 上に実装する (配送結果のマージ更新)
...
**Lock マネージャ(lease 取得・奪取 / heartbeat / 解放 / stale 回復)**

- **責務**: lock ファイル(lease レコード: hostname + boot-id + acquired_at + renewed_at + ttl + generation。構造化スキーマは object-storage-schema.yaml schemas.lock_lease)の取得・奪取・解放、active な serve の heartbeat による renewed_at 更新(**更新前に lock を read して hostname/boot-id が自分自身(self)か、かつ generation が自分が最後に書いた値と一致するかを確認し(generation CAS)、両方一致時のみ renewed_at と generation(+1)を更新する。不一致=他ノードが奪取済み(generation を進めている)なら ErrLeaseLost を返し runtime が自発降格する。これにより旧 active が新 active の lease を check→update の隙に上書きで奪い返す TOCTOU 経路を generation 不一致で検出する。奪取側は観測した generation を進めて書く**)、`現在時刻 - renewed_at > ttl` による stale 判定・安全な回復(奪取)、lease 保持確認(メッセージ境界 lease 確認に用いる)を提供する(SR-006、LR-002、SPEC-015-01)。stale 判定は PID 生存確認に依存せずマルチホストで他ホストの旧 active を判定できる。stale lease の奪取は **read → remove → O_CREATE|O_EXCL で再作成(boot-id 更新)** の順で行い、複数 standby が同時に奪取しても O_EXCL で「最終的に1人だけ」が勝ち、敗者は standby 待機を継続する(旧 active の heartbeat 書込と競合しても二重所有にならない)。有効な lease を他ホストが保持していれば standby 待機を返し、同一ホスト・同一構成の二重起動では取得失敗を返して呼び出し元(runtime)が終了コード 3 で終了する。唯一性保証方式(方式B=lease 自動奪取 / 方式A=外部クラスタ委譲)で起動モデルが異なる: 方式B は有効 lease で standby 待機・stale で自動奪取。方式A は外部クラスタ起動時に常に active として起動し lease は観測用に書く(有効 lease でも standby に落ちず、hostname/boot-id が自分と異なれば奪取)、TTL 失効による自動奪取は行わない(spec-decision-009)。設定キーは `high_availability.uniqueness_method`(lease / external_cluster)・`high_availability.lease_ttl`(秒、既定 90)・`high_availability.heartbeat_interval`(秒、既定 lease_ttl/3=30)。`high_availability` 省略時は単一インスタンス運用で従来の PID 相当の二重起動防止(ui-design.md・tier-daemon-worker.md と同一キー)。**既知の制約**: NFS では O_CREATE\|O_EXCL・read/write の原子性が実装・バージョン依存であり、lease の奪取(O_EXCL)・generation CAS でも完全な分散排他は保証できない。本コンポーネントは実務上の原子性 + TOCTOU 検出強化を提供するが exactly-once は保証せず、残る split-brain の窓の被害は冪等 I/O(spec-decision-010)とメッセージ境界 lease 確認 + Manifest 直列化(spec-decision-011)で限定する
- **配置レイヤー**: `L-daemon-gateway`(lease ファイル I/O・stale 判定。利用は `L-daemon-runtime` の起動 / heartbeat / 停止シーケンス)
- **利用する UC**:
  - [デーモンを起動する](../../配信基盤運用業務/配信基盤を運用するフロー/デーモンを起動する/tier-daemon-worker.md) — 起動時の lease 取得・奪取・heartbeat・stale 回復・standby 待機・active 昇格/降格・同一ホスト二重起動検出
  - [冪等に処理を再開する](../../配信基盤運用業務/配信基盤を運用するフロー/冪等に処理を再開する/tier-daemon-worker.md) — standby → active 昇格(別ホストのクラッシュ再開と等価)後の lease 保持
  - [デーモンをgraceful shutdownで停止する](../../配信基盤運用業務/配信基盤を運用するフロー/デーモンをgraceful shutdownで停止する/tier-daemon-worker.md) — 停止時の Lock 解放
- **主要インターフェース案**:

```go
// gateway/lock
var ErrAlreadyLocked = errors.New("valid lease held by another instance") // 同一ホスト二重起動 → 終了コード 3 / 他ホストは standby 待機
var ErrLeaseLost = errors.New("lease no longer owned by self") // heartbeat/保持確認で他ノードが奪取済みを検知 → 自発降格 (稼働中 → standby待機)

// Lease は lock ファイルへ記録する lease レコード
type Lease struct {
	Hostname   string
	BootID     string
	...
	RenewedAt  time.Time
	TTL        time.Duration
	Generation int64 // 世代カウンタ。取得・奪取で +1。heartbeat の generation CAS と TOCTOU 検出に用いる
}

type LockManager struct{ path string }

// Acquire は lease を取得する。stale lease (renewed_at + ttl 超過) は read→remove→O_CREATE|O_EXCL で
// 安全に奪取して再取得する (boot-id 更新)。複数 standby 同時奪取は O_EXCL で勝者 1 人に収束する
func (l *LockManager) Acquire(self Lease, now time.Time) error
// Heartbeat は lease の renewed_at を更新する (active のみ、heartbeat 間隔ごと)。
// 更新前に lock を read し、hostname/boot-id が self と一致 かつ generation が self.Generation と一致
// するか確認する (generation CAS)。両方一致する場合のみ renewed_at と generation(+1) を更新する。
// 不一致 (他ノード/他世代が lease を奪取済み=generation が進んでいる) なら ErrLeaseLost を返し、
// 呼び出し元 (runtime) は自発降格 (稼働中 → standby待機) する。旧 active が新 active の lease を
// check→update の隙に上書きで奪い返す TOCTOU 経路を generation 不一致で検出する (spec-decision-009)。
// NFS の原子性は実装依存のため完全な分散排他は保証しない (既知の制約。exactly-once は保証しない)
func (l *LockManager) Heartbeat(self Lease, now time.Time) error
// HoldsLease は自身が lease を保持しているか確認する (hostname/boot-id が自分自身か、かつ ttl 以内か)。
// メッセージ境界 lease 確認に用い、失っていればその 1 メッセージで停止・降格する (spec-decision-011)
func (l *LockManager) HoldsLease(self Lease, now time.Time) (bool, error)
// Release は lock ファイルを削除して解放する (graceful shutdown 時)
func (l *LockManager) Release() error
// isStale は renewed_at + ttl 超過による stale 判定 (内部。PID 生存確認に依存しない)
```
...
- **責務**: 単一 YAML 設定(CTP-003)の読込・パース、認証情報の環境変数参照 `${ENV_VAR}` / 鍵ファイルパスの解決(CTP-002)、構文・必須キー・列挙値・参照整合(Topic↔Subscription↔収集ソース↔認証情報参照)・名前重複の検証(SR-101)を提供する。冗長化設定 `high_availability`(uniqueness_method / lease_ttl / heartbeat_interval)の読込・既定値補完・検証も担う: ブロック省略時は単一インスタンス運用(補完・HA 検証なし)、present 時は uniqueness_method=lease / lease_ttl=90 / heartbeat_interval=lease_ttl/3 を既定として補完し、列挙値・正の整数・lease_ttl > heartbeat_interval・lease_ttl > actimeo(満たさねば警告)を検証する(SPEC-015-02/017-01)。検証 NG は「YAML キーパス + 原因 + 対処」を持つエラーとして返し、呼び出し元が終了コード 2 に変換する。デーモン起動・CLI 全サブコマンドが共有する
- **配置レイヤー**: `L-daemon-gateway`
- **利用する UC**:
...
type Config struct {
    ...
    HighAvailability *HighAvailability   `yaml:"high_availability,omitempty"` // 省略時 = 単一インスタンス運用 (lease 化しない)
}

// HighAvailability は active/standby 冗長化設定。present 時のみ lease で動作する
type HighAvailability struct {
    UniquenessMethod string `yaml:"uniqueness_method"` // lease (方式B) / external_cluster (方式A)
    LeaseTTL         int    `yaml:"lease_ttl"`         // 秒。省略時 90
    HeartbeatInterval int   `yaml:"heartbeat_interval"` // 秒。省略時 lease_ttl/3
}

// Load は YAML を読込み、${ENV_VAR} を展開し、HighAvailability の既定値補完
// (present 時: uniqueness_method=lease / lease_ttl=90 / heartbeat_interval=lease_ttl/3) を行い、
// Validate を通して返す。high_availability ブロック省略時は補完せず単一インスタンス運用とする
func Load(path string) (*Config, error)

// Validate は構文・必須キー・列挙値・参照整合・名前重複を検証する。
// high_availability が present の場合は uniqueness_method が lease/external_cluster のいずれか・
// lease_ttl が正の整数で heartbeat_interval より十分大きいこと・lease_ttl が actimeo (既定 60s) より
// 十分大きいこと (満たさねば警告) を検証する。省略時は HA 検証をスキップ (単一インスタンス)
