...
topics:
  - name: orders            # Topic 名(論理的なファイル種別)
    ...
    source:
      type: sftp            # ソース種別: ftp / sftp / scp / local
      host: legacy-host01   # 接続先ホスト(local の場合は不要)
      directory: /out/orders        # 対象ディレクトリパス
...
| 認証情報の推奨記法 | 環境変数参照 `${ENV_VAR}` と鍵ファイルパス指定を推奨。YAML 平文も許容するが README で注意喚起する | CTP-002、バリエーション「認証方式」 |
| 起動前検証 | 編集後は必ず `config validate` で構文・参照整合(Topic↔Subscription↔収集ソース↔認証情報参照)を検証してから serve する | SR-101 |
| ソース種別非依存 | source の type を切り替えても subscriptions 以下や後段(Archive/Fan-out/Manifest)の定義は変わらない | LP-301、情報「収集ソース」 |
