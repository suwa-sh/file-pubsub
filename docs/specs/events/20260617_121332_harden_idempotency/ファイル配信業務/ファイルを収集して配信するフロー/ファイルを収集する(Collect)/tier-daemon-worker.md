...
3. domain の判定ルールを適用する(LR-204):
   - 除外パターン該当 → 対象外
   - 完了検知方式(completion.mode)に従い書き込み完了判定:
     - 安定判定(stability): サイズ・更新時刻の安定待ち(pull 型と同一ロジック)
     - rename: 一時拡張子(completion.suffix。既定 .tmp、設定可能)で終わらない正式名のみ取り込み対象。一時名(例 xxx.csv.tmp)は対象外
     - done マーカー(marker): 対象 xxx に対応するマーカー xxx+suffix(suffix=completion.suffix。既定 .done、設定可能)が存在すれば xxx を取り込み対象にする。マーカー自体は配信対象に含めない。ただし copy 設定で残置されたマーカーが既に処理済み管理に記録済みの場合は新たな完了契機にしない(同名で内容の異なる本体の再 put は、新しい未処理マーカーが置かれるまで取り込まない — SPEC-014-02)
...
5. message_id を採番する(pull 型と同一。LR-202)。同一収集秒に同名ファイルを複数回収集して message_id が衝突する場合は末尾に連番(_2, _3 …)を付与し、先行メッセージの Archive・Manifest を上書きしない(SPEC-007-01)。fsnotify とフォールバックポーリングが同一ファイルを二重検知しても、処理済み管理照合と合わせて二重取り込みしない
...
- 書き込み完了判定: 書き込み中のファイルは収集しない。完了検知方式 completion.mode(stability / rename / marker)を収集ソース設定で選択し、domain 層の共通ルールとしソース種別・収集モードに依存させない(SP-003、LR-204)。既定は安定判定。rename/marker の判定に使うサフィックスは completion.suffix で Producer 規約に合わせ設定可能(既定 rename=.tmp / marker=.done — SPEC-014-03)
...
Feature: ファイルを収集する(Collect) - 常駐デーモン
  ...
  Scenario: push 受信モードでイベント駆動とフォールバックの両方で取りこぼさず冪等に取り込む
    Given topic「invoices」の収集ソースが type=inbox, directory=/inbox/invoices, completion.mode=stability で設定されている
