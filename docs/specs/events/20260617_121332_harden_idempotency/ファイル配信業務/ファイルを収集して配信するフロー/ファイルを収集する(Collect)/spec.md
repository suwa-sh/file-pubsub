...
| 元ファイル処理判定 | 回収(既定)。pull 型は GET 後 DELETE、push 受信モードは受信ディレクトリから削除(done マーカー方式では xxx.done も削除)。Topic 設定で「残す(copy)」選択時は処理済み管理と照合し、処理済みファイル(push の done マーカー残置含む)は再収集しない。done マーカー・copy では既に処理済みのマーカーを新たな完了契機にしない(同名で内容の異なる本体の再 put は新しいマーカーが置かれるまで取り込まない。SPEC-014-02) | tier-daemon-worker | usecase 収集サイクル / 処理済み管理 / 収集コネクタ(マーカー後始末 LR-305) | copy 設定で同じファイルを重複収集しない / done マーカーの後始末 / 処理済みマーカーで再 put を即収集しない |
...
|--------|---------|---------------|---------|----------|
| message_id 採番 | メッセージ(収集時刻、Topic名、元ファイル名)、既存メッセージ | 収集時刻(秒精度) + Topic + 元ファイル名を結合して採番(例: 20260612T093001_orders_orders_20260612.csv)。同一収集秒に同名ファイルを複数回収集して message_id が衝突する場合は末尾に連番(_2, _3 …)を付与して一意化し、先行メッセージの Archive・Manifest を上書きしない(SPEC-007-01) | メッセージ.message_id | tier-daemon-worker |
| 安定判定 | 収集ソース(安定待ち判定設定)、元ファイル候補(サイズ、更新時刻) | 安定確認間隔をおいて 2 回観測しサイズ・更新時刻が一致すれば安定(完了検知方式=安定判定の場合) | 収集可能 / 書き込み中 の判定 | tier-daemon-worker |
| rename 判定 | 収集ソース(完了検知方式=rename、completion.suffix)、元ファイル候補(ファイル名) | ファイル名が一時拡張子(completion.suffix。既定 .tmp)で終わらなければ取り込み対象。一時名(例 xxx.csv.tmp)は対象外。suffix は Producer 規約に合わせ設定可能(.part 等) | 収集可能 / 対象外 の判定 | tier-daemon-worker |
| done マーカー判定 | 収集ソース(完了検知方式=marker、completion.suffix)、元ファイル候補(ファイル名)、マーカー(xxx + suffix) | 対象ファイル xxx に対応するマーカー xxx+suffix(既定 .done、設定可能)が存在すれば xxx を取り込み対象にする。マーカー自体は配信対象に含めない | 収集可能 / マーカー未着 の判定 | tier-daemon-worker |
...
Feature: ファイルを収集する(Collect)
  ...
  Scenario: push 受信モード(inbox)で受信ディレクトリへ put されたファイルを取り込む
    Given topic「invoices」の収集ソースが type=inbox, directory=/inbox/invoices, original_file_handling=delete, completion.mode=stability で設定されている
  ...
  Scenario: rename 方式で正式名の出現を契機に取り込む
    Given topic「invoices」の push 受信モードが completion.mode=rename で設定されている
  ...
  Scenario: rename 方式で Producer 規約のサフィックス(.part)を設定して取り込む
    Given topic「invoices」の push 受信モードが completion.mode=rename, completion.suffix=.part で設定されている
    When Producer が「invoices_0045.csv.part」で書き込み後「invoices_0045.csv」へ rename する
    Then 正式名「invoices_0045.csv」の出現を契機に取り込まれる
    And 一時名「invoices_0045.csv.part」は取り込み対象とならない

  Scenario: done マーカー方式でマーカーの出現を契機に取り込みマーカーを後始末する
    Given topic「invoices」の push 受信モードが completion.mode=marker, original_file_handling=delete で設定されている
  ...
  Scenario: done マーカー方式で Producer 規約のサフィックス(.ok)を設定して取り込む
    Given topic「invoices」の push 受信モードが completion.mode=marker, completion.suffix=.ok, original_file_handling=delete で設定されている
    When Producer が「invoices_0046.csv」を put した後に「invoices_0046.csv.ok」を put する
    Then 「invoices_0046.csv.ok」の出現を契機に「invoices_0046.csv」が取り込まれる
    And マーカー「invoices_0046.csv.ok」自体は配信対象とならない
    And 取り込み後に「invoices_0046.csv」と「invoices_0046.csv.ok」の双方が受信ディレクトリから削除される
  ...
  Scenario: 同一収集秒に同名ファイルを再収集しても履歴を失わない
    Given topic「invoices」の push 受信モードで「invoices_0042.csv」が message_id「20260617T020637_invoices_invoices_0042.csv」として収集済みである
    And 同じ収集秒(20260617T020637)内に同名の「invoices_0042.csv」が再び put され取り込まれる
    When 収集が実行される
    Then 連番付与で別の message_id「20260617T020637_invoices_invoices_0042.csv_2」が採番され、先行メッセージの Archive・Manifest は上書きされない
```
...
```gherkin
  ...
  Scenario: push 受信モード・copy 設定で処理済みファイルを再取り込みしない
    Given topic「invoices」の push 受信モードが completion.mode=marker, original_file_handling=copy で「invoices_0048.csv」が処理済み管理に記録済みである
    And 「invoices_0048.csv」と「invoices_0048.csv.done」が受信ディレクトリに残ったまま次の取り込み契機が来る
    When 取り込み契機が走る
    Then 「invoices_0048.csv」は再取り込みされず重複メッセージは発生しない

  Scenario: marker・copy 設定で残存マーカーは新本体の完了契機にならない
    Given topic「invoices」の push 受信モードが completion.mode=marker, original_file_handling=copy で「invoices_0048.csv」と処理済みの「invoices_0048.csv.done」が残置されている
    When Producer が同名で内容の異なる「invoices_0048.csv」を再 put する(新しいマーカーはまだ置かない)
    Then 既存の処理済みマーカー「invoices_0048.csv.done」では取り込まれず、Producer が新しい「invoices_0048.csv.done」を置いてから取り込まれる
